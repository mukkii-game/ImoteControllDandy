...
export const BRO = {
  ...
  /**
   * 地上の電撃：ボタン不要。サイトが敵の弾か敵を捉えると自動で兄の手から山なりの電撃が伸び、当て続けてダメージ（弾が優先）。
   * 敵が動いても着弾点は追いかけ、倒れるまで続く（次にサイトに入った敵へ移る）。サイトから外れると消える。乗っている敵は狙わない
   */
  lightning: {
  ...
  roofJump: {
    ...
    aimRadius: 0.06,
    rays: 5,
    /** この高さ未満の建物（住宅）は選ばない（m。ビルだけ）。住宅の上に落ちた時はめり込まず上に立つ（floorAt） */
    minHeight: 13,
    /** サイトが重なったビルの輪郭の色 */
  ...
  /**
   * 敵に乗る：サイトが敵に重なるとその敵の輪郭が光り、A でその敵の上へ山なりに跳んで乗る。敵と一緒に動き、敵が死ねば落ちる。
   * 乗っている間もサイトの敵へ電撃は出る。A（タックル）で降りる。B で妹へ。エネミービル（boss）には乗れない
   */
  ride: {
    enabled: true,
    /** サイトからこの半径（画面高さ比）の敵を選ぶ */
    aimRadius: 0.09,
    /** 跳び乗るまでの秒数と弧の高さ（m） */
    sec: 0.9,
    arcUp: 8,
    ...
    /** 乗っている間のカメラ：視点入力が止まってからこの秒数で妹の方へ戻る（戻る速さ） */
...
export const CAMERA = {
  ...
  /** タッチをマウスと同じ「ずらした分だけ回る」で使う時の感度（touchStick.enabled=false の時だけ） */
  touchSensitivity: 0.006,
  /**
   * タッチの視点操作は仮想右スティック：指を置いた所が中心、そこからのずれが「回る速さ」。
   * マウスと違って指は画面端で止まるので、ずらした量ではなく速さにしないと少ししか回れない。
   * 溜め中はサイトが、地上はサイト（左右）とカメラ（上下）が同じ速さで動く
   */
  touchStick: {
    enabled: true,
    /** この px ずらすと最大速度（円の半径） */
    ...
    /** 中心付近のあそび（radius 比） */
    deadZone: 0.12,
    /** 最大に倒した時のカメラ回転速度（rad/s） */
    yawSpeed: 2.6,
    pitchSpeed: 1.8,
    /** 最大に倒した時のサイトの移動速度（px/s） */
    reticleSpeed: 900,
    /** 倒し量→速さのカーブ（1=比例、2=中心付近を細かく） */
    curve: 1.6,
  },
  pitchMin: -0.9,
  pitchMax: 1.1,
...
export const QUALITY = {
  ...
  auto: { warmupSec: 3, measureSec: 5, lowBelow: 25, midBelow: 45 },
  maxWidth: 1920,
  maxHeight: 1080,
  presets: {
