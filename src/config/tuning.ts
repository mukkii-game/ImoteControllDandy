...
export const TUNE_GROUPS: { title: string; items: TuneItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: 'CAMERA.ground.dashFollowLerp', label: 'ダッシュ中の追従の速さ（小さいほど遅れる）', min: 0.5, max: 12, step: 0.1 },
      { path: 'CAMERA.ground.dashCam.peekPitch', label: '着地を上から見る角度', min: 0, max: 1, step: 0.02 },
      { path: 'CAMERA.ground.dashCam.peekSec', label: '上から見る秒数', min: 0, max: 1, step: 0.05 },
      { path: 'CAMERA.ground.dashCam.peekRecoverSec', label: '横向きに戻す秒数', min: 0.05, max: 1.5, step: 0.05 },
